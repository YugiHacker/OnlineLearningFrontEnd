...
export default function IndividualComment(props) {
    ...
    function toggleUpdate() {
        ...
        React.useEffect(() => {
            const response = async () => {
                const loadData = await request.get('CommentVote/userId && commentId?userId=' + props.userIdVote + '&commentId=' + props.commentId)
    ...
    return (
        <div>
            <p className="comment--date"> created on {date[0]} {date[1]}</p>
            <div className="comment--body">
                <img src="user" className="comment--image" alt="User" />
